...
// Description  : Implements a resonant low pass filter, a biquad filter.
//      Implements the bilinear transform to convert from analog coefficients
//      to digital ones.
//
//------------------------------------------------------------------------------
...
void ResonantLowPass::setParameter(VstInt32 index, float value)
{
	...
	case kParamQ:
		...
		QValue = SmartKnob::knob2value(QKnob, QLimits, QTaper);
			...
			// design new input filter & update the coefficients
			designResonantLowPass(lpCoefs, FcValue, QValue);
			lpFilter.setCoefs(lpCoefs);
...
void ResonantLowPass::bilinearTranform(float acoefs[], float dcoefs[])
{
    ...
    float c, alpha, beta; // for bi-linear transform
